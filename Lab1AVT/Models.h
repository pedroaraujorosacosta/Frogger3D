...
class objLoader{
        ...
        public:
        objLoader();   
        ~objLoader();										//free the textures
        void load(const char* filename, float **vertices,   //the main model load function
			float **normals, float **texCoords, unsigned int **faceIndex, 
			int *verticeCount, int *faceCount);					
		unsigned int loadTextureSky(const char* filename);	//public load texture function
};
