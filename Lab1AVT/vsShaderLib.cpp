...
VSShaderLib::loadShader(VSShaderLib::ShaderType st, std::string fileName) {
	...
	if (s != NULL) {
		...
		checkShaderCompilation(pShader[st]);

		free(s);
	}
}

void VSShaderLib::checkShaderCompilation(GLuint shaderId)
{
	GLint isCompiled = 0;
...
void VSShaderLib::checkProgramLinkage(GLuint programId, GLuint vertexShaderId, GLuint fragmentShaderId)
{
	GLint isLinked = 0;
